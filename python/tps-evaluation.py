...
def plot_box_plot(results):
    ...
    ax.boxplot(results, labels='tps', widths=0.5)
...
async def main():
    ...
    # Web3.py instance, Cloud Blockchain
    rpc_url = 'http://bc-vmikro.eastus.cloudapp.azure.com:8545'
    ...
    for odom in odom_data:
        begin = time.time()
        tasks = [transact(w3, contract, fromAddress, contractAddress, privateKey, odom) 
				 for _ in range(tps)]
        await asyncio.gather(*tasks)
        await asyncio.sleep(1)  # Waiting for 1 second before starting the next set of transactions
